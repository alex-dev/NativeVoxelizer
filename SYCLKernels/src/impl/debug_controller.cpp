...
namespace native_voxelizer::sycl_kernels::impl {
  ...
  debug_controller::vector_collection
      debug_controller::get_debug_vertices(guid const& id, debug_controller::allocator&& allocator) const {
    auto* const input_buffer = meshes_controller.get_mesh(id);
    size_t length = input_buffer ? input_buffer->get_count() : 0;
    vector_collection output{length, std::forward<debug_controller::allocator>(allocator)};

    if (length) {
      sycl::buffer<vector3<float>, 1> output_buffer = {output.data(), sycl::range<1>{length}};
      queue
          .submit([& output = output_buffer, &input = *input_buffer](sycl::handler& command_group) mutable {
